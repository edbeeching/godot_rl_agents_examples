...
func get_spawn_location() -> Vector3:
	return get_random_location(ground.size.y/2.0 + _character_aabb.size.y / 2.0)


func get_target_location() -> Vector3:
	return get_random_location(ground.size.y/2.0 + _target_aabb.size.y / 2.0)


func get_random_location(y_offset := 1.0) -> Vector3:
	## Taken from get_target_location, may need adjustments
	return Vector3(
